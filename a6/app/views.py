...
class ProductViewSet(viewsets.ModelViewSet):
    queryset = Product.objects.filter(Q(deleted=0)|Q(deleted=None)).all().prefetch_related('category') # pass this only for GET, PUT, PATCH requests, not for DELETE
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if orderSerializer.is_valid():
            validated_data = orderSerializer.data
            ...
            status_str = validated_data['status']
...
def contact(request):
    ...
    user = User(company_name=uname)
    user.save()
    feedback = Feedback(user=user, timestamp=datetime.datetime.now(), text=msg)
    feedback.save()
    return HttpResponse(status=200)
